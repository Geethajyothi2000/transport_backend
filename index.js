...
let {id,v_no,vname,kmwith_dr,with_dr_rent,kmwithout_dr,withoutdr_rent,v_total} = req.body
  ...
  console.log(error)   
   ...
   let {en,ename,mearnings,yearnings,type} = req.body;
    ...
    try {
     const result =await db.query("INSERT INTO employees (en,ename,mearnings,yearnings,type) VALUES ($1,$2,$3,$4,$5)",[en,ename,mearnings,yearnings,type]);
     //console.log(result.rows)
     res.json("success")
 } catch (error) {
  console.log(error)   
 ...
 yearnings = mearnings * 12;
    try {
     const result =await db.query("UPDATE employees SET en=$2, ename=$3, mearnings=$4, yearnings=$5, type=$6  WHERE id = $1",
  ...
  console.log(error)   
   ...
   app.post('/addEmployee2', async function (req, res) {
    ...
    let {vname,vnumber,model,ipremium,aservicecharge,emission} = req.body;
    
     try {
      const result =await db.query("INSERT INTO vehicle (vname,vnumber,model,ipremium,aservicecharge,emission) VALUES ($1,$2,$3,$4,$5,$6)",[vname,vnumber,model,ipremium,aservicecharge,emission]);
      //console.log(result.rows)
      res.json("success")
  } catch (error) {
   console.log(error)   
   ...
   console.log(error)   
    ...
    app.post('/Vupdate', async function (req, res) {
     ...
     console.log(req.body)
  let {id,vname,vnumber,model,ipremium,aservicecharge,emission} = req.body

     try {
      const result =await db.query("UPDATE vehicle SET vname=$2, vnumber=$3,model=$4, ipremium=$5, aservicecharge=$6, emission=$7 WHERE id = $1",
      [id,vname,vnumber,model,ipremium,aservicecharge,emission])
     //  console.log(result.rows[0])
     //  console.log(result.rows)
      res.json("success")
